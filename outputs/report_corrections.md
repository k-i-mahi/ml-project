...
`member` (collector) and `region` are **perfectly 1:1** — each of the 6 collectors covered exactly one region. Load-time medians run 8.07 s (M2/Western Europe) to 12.94 s (M6/Latin America & Africa), and that gradient cannot be attributed to websites rather than to collectors. Load time is therefore used only in region-standardised form, every ranking is produced with and without it (Spearman between the two: 0.9925), and the confound is reported as unresolvable rather than adjusted for.

## 8. Where the report's weighting intuitions did not survive measurement

| block | declared in rubric_v1 | realised (SHAP) | ratio |
|---|---|---|---|
| B7_visual_design | 3.4% | 5.9% | 1.74× |
| B3_notices_updates | 12.7% | 16.7% | 1.32× |
| B1_header_nav | 11.5% | 13.6% | 1.19× |
| B5_page_content | 37.9% | 43.5% | 1.15× |
| B2_rankings_recog | 2.1% | 2.4% | 1.12× |
| B9_technical_perf | 6.2% | 5.4% | 0.86× |
| B4_events_media | 8.9% | 6.4% | 0.72× |
| B6_footer | 5.4% | 3.8% | 0.70× |
| B11_accessibility | 6.4% | 1.3% | 0.20× |
| B8_service_interact | 1.8% | 0.3% | 0.18× |
| B10_seo_metadata | 3.6% | 0.6% | 0.18× |
