...
| | |
|---|---|
| Rank correlation between the rubric and blind judgment | **ρ = 0.790** — correlated, not collapsed |
| Judgment self-consistency (60 swapped repeats) | **86.7%** |
| Best model, leave-one-region-out | **ρ = 0.844** (LightGBM+mono) |
| Transparent rubric baseline, same folds | ρ = 0.708 |
| Accessibility: declared vs. realised weight | 6.4% → 14.8% (**2.3×**) |

## What was actually found

**1. A weighted-sum rubric is a decent proxy and a poor judge.** Track A predicts 77% of blind pairwise judgments overall — but 86% on pairs it already sees as far apart and only **63% on close pairs**, barely above chance. A presence-counting formula cannot tell apart two sites with similar feature counts and different execution. That gap is the entire justification for learning a model rather than publishing the rubric.

**2. Machine learning added something measurable here.** LightGBM+mono beats the calibrated rubric on all 25 nested-CV fold-reps (ρ 0.881 vs 0.791, paired p < 0.0001) and holds the margin under leave-one-region-out (0.844 vs 0.708). This was not guaranteed and the comparison was built to be able to say the opposite.

**3. Hand-set weights misallocate importance — measured, not asserted.** Accessibility was declared at 6.4% of the rubric and drives 14.8% of the ranking (2.3× its brief). Technical performance was declared at 6.2% and drives 2.8% (0.45×) — the same direction as the published Rashida et al. failure (40% declared, 2.9% realised), caught early here by auditing block variance before fixing any weight.

**4. Technical metrics alone are close to useless for this task.** A model using only the technical block reaches ρ = 0.069 under LORO. Page speed, HTTPS and mobile scores are near-universal, so they cannot separate anything.
...
| `artefact_inventory.csv` | all 107 files with sha256 |
...
2. **One rater.** Self-consistency is measured (86.7% on 60 swapped repeats). Inter-rater agreement is not measured because there is no second rater. This is the single largest threat to validity and no statistic here addresses it.
...
6. **n = 200 labelled of 1,226.** The other 1,026 are pure inference. The labelled and inferred prediction distributions are statistically compatible (KS p = 0.09), so the model is interpolating rather than extrapolating — but the confidence intervals on the 1,026 are wider than any number here shows.
